...
%======================================================================
function [materialProps]=materialPrompt()
    
    promptMaterial = ({'Young modulus:','Poisson ratio:','Thickness:'});
    definput={'210000000000','0.3','0.001'};
    
    WAIT_USER_INPUT=true;
    while WAIT_USER_INPUT
        materialProps = inputdlg(promptMaterial,'Material properties',[1 35],definput);
        if ~isempty(materialProps)
            checkAnswer(materialProps);
        else
